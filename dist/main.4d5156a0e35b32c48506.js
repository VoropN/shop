...
eval("module.exports = \"<div class=row> <div class=\\\"two column block-img\\\"><img src={{url}} alt=none></div> <div class=\\\"two column\\\"> <div><span>Name: </span>{{ name }}</div> <div><span>Price: </span>{{ price }}</div> <div><span>Gender: </span>{{ gender }}</div> <div *if=fur><span>Fur: </span>{{ fur }}</div> <div><span>Color: </span>{{ color }}</div> <button class=\\\"button button-primary\\\">Add</button> </div> </div>\";\n\n//# sourceURL=webpack:///./components/product/product.html?");
